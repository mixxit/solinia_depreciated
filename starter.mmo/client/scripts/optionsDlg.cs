...
function optionsDlg::setPane(%this, %pane)
{
   ...
   OptGraphicsAdvancedPane.setVisible(false);
...
function OptionsDlg::onWake(%this)
{
   ...
   OptGraphicsShaderQuality.setValue( $pref::Material::qualityLevel );
   OptGraphicsWaterReflectionSize.init();
   ...
   OptGraphicsShowFloatingText.setValue($pref::Video::ShowFloatingText);
   OptGraphicsShowSelectrons.setValue($pref::Video::ShowSelectrons);
...
function optionsDlg::onShaderQualitySlider( %this )
{
   $pref::Material::qualityLevel = mFloor( OptGraphicsShaderQuality.getValue() );
   if ( isTMKenabled() )
   		MaterialManager::setShaderQuality( $pref::Material::qualityLevel );
}


function OptGraphicsWaterReflectionSize::init( %this )
{
   %this.clear();
   ...
   {
      %this.add( %dimension @ " x " @ %dimension, %index );
      
      if ( %dimension == $pref::Water::reflectionSize )
         %selIndex = %index;
...
function OptGraphicsWaterReflectionSize::onSelect( %this, %id, %text )
{
   $pref::Water::reflectionSize = getWord( %text, 0 );
   if ( isTMKenabled() )
   		MKFluid::setReflectionSize( $pref::Water::reflectionSize );
}
