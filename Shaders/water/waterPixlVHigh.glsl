...
uniform vec3 EyePos;
uniform sampler2D reflectionTex, NormalMap, DepthTexture, BaseTexture, RefractTexture, ShoreTexture;
...
void main()
{
   ...
   // Normalize the normal, do fresnel calcs
   normal = normalize(normal);
   half fresnel = dot(eyeVec, normal);
   // Base water color
   half3 waterColor = h3tex2D(BaseTexture, gl_TexCoord[0].st).rgb;
   half3 shoreColor = h3tex2D(ShoreTexture, gl_TexCoord[0].st).rgb;
   waterColor = lerp(shoreColor, waterColor, depthAlpha);
   
   fresnel = FresBias + (1.0-FresBias)*pow(1.0-max(fresnel, 0.0), FresPower);
   
   // Get our base water color, include reflection
   refracted.rgb = lerp(refracted.rgb, waterColor, depthAlpha);
   
    fresnel = max(0.0, fresnel - 0.5) * 2.0;
   
   half alpha = (depthAlpha) + fresnel;
   
   // Toss in refraction
   reflected = lerp(reflected.rgb, waterColor.rgb, 0.24);
   reflected = lerp(refracted.rgb, reflected, alpha);
   
   // Specular calcs!
   half3 halfVec = normalize(half3(HalfVec));
   half specular = dot(halfVec, normal);
   specular = max(specular, 0.0);
   half4 specColor = SpecularColor * pow( specular, SpecularPower);
   specColor *= (specColor.a * alpha);
   // If you value your framerate, don't uncomment this.  If you like specular, go for it.
   reflected += specColor.rgb;
