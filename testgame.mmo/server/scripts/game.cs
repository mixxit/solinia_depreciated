...
{
   $PlayerGroup = new SimGroup(PlayerGroup);
   ...
   exec("./rpg/bots.cs");
   exec("./rpg/zonetrigger.cs");
   exec("./rpg/projectiles.cs");
...
//this is called after world says so
function GameConnection::onClientEnterGameReal(%this,%transform,%datablock)
{
   ...
   %this.createPlayer(%transform,%datablock);
...
{
   //tell the world we are ready to go
   Py::PyClientEnterGame(%this);
...
function GameConnection::createPlayer(%this, %spawnPoint,%datablock)
{
   if (%this.player > 0)  {
...
function Show3dDamage( %shape, %damageStr, %r, %g, %b )
{
//    echo( 'damage string = ' @ %damageStr );

    // Inform the clients to print damage message
    for ( %clientIndex = 0; %clientIndex < ClientGroup.getCount(); %clientIndex++ )
    {
        %cl = ClientGroup.getObject( %clientIndex );
        commandToClient( %cl, 'NewDamagePopup', %cl.getGhostId(%shape), %damageStr, %r, %g, %b );
    }
}
